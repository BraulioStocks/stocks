...
# --- Annual Valuation Metrics (Bar Chart) -----------
with st.expander("📊 Annual Valuation Metrics (Bar Chart)"):
    try:
        ...
        bs = t.balance_sheet
        if bs is None or bs.empty or 'TotalStockholderEquity' not in bs.index:
            raise ValueError("Missing balance sheet data")
        equity = bs.loc['TotalStockholderEquity']
        ...
        df_ratios = pd.DataFrame({'P/E': pe, 'P/S': ps, 'P/B': pb}, index=ye_price.index)
